...
PTS2 = np.float32([[0, 0], [AR_WIDTH, 0], [AR_WIDTH, AR_HEIGHT], [0, AR_HEIGHT]])
...
def get_2d_point(centroid, points1, points2):
    ...
    try:
        ...
        pts = [points1[0], points1[3], points1[2], points1[1]]
        homo, status = cv2.findHomography(pts, points2)
...
def pt_in_roi(cx, cy, roi, mode='poly'):
    print(type(cx))
    if mode == 'rect':  # roi: [x, y, w, h]
        if cx < roi[0]:
            return False
        elif cx > roi[0] + roi[2]:
            return False
...
if __name__ == '__main__':
    ...
    mapname = '2D Map'
    pixel_per_unit = 40
    PTS1 = np.int32([[0, 0], [cam.get(3), 0], [0, cam.get(4)], [cam.get(3), cam.get(4)]])
    ...
    while cv2.getWindowProperty(name, cv2.WND_PROP_VISIBLE)>=1:
        ...
        if ui_control.status == 'select' and play:
            play = play ^ 1
            selecting = True
            roi_color = (0, 255, 255)
        elif ui_control.status == 'init' and selecting:
            play = play ^ 1
        ...
        if map_opened or cv2.getWindowProperty(mapname, cv2.WND_PROP_VISIBLE)>=1:
            ...
            if not pt == (-1, -1):
                ...
                if len(roi) < 3:
                    pass
                elif pt_in_roi(pt[0], pt[1], roi, mode=mode):
        ...
        if ui_control.new_init:
            ...
            if mode == 'rect':
                ...
                PTS1 = np.int32(state)
        if mode == 'rect':
            cv2.rectangle(vis, (state[0], state[1]), (state[2] + state[0], state[3] + state[1]), roi_color, 5)
        elif mode == 'poly':
    ...
    ##### test cv2.polylines() #####
    # black = np.zeros((AR_HEIGHT, AR_WIDTH, 3), np.uint8)
    # cv2.namedWindow('test', cv2.WINDOW_KEEPRATIO)
    # pts = np.array([[25, 70], [25, 160],
    #                 [110, 200], [200, 160],
    #                 [200, 70], [110, 20]], np.int32)
    # pts2 = np.array([[25, 70], [25, 145],
    #                 [75, 190], [150, 190],
    #                 [200, 145], [200, 70],
    ...
    # black = cv2.polylines(black, [pts, pts2], True, (255,255,255), 8)
